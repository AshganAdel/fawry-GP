...
def test_validate_login_exception(client, monkeypatch):
    """
    Tests the exception handling in the validateLogin route.
    Instead of breaking cursor creation, we make cursor.callproc raise an exception.
    """
    fake_con = mock.MagicMock()
    fake_cursor = mock.MagicMock()
    fake_cursor.callproc.side_effect = Exception("Test DB Error")
    fake_con.cursor.return_value = fake_cursor

    monkeypatch.setattr("app.mysql.connect", lambda: fake_con)
